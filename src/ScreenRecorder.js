...
async function setupMediaStream() {
    ...
    recorder.onstop = async function () {
        ...
        console.log("In renderer: " + mostRecentChunk);
        blob = new Blob(mostRecentChunk, { 'type': 'video/mp4' });
        let milliseconds = seconds * 1000;
        fixedBlob = await webmFixDuration(blob, milliseconds, "video/mp4");
        videoURL = window.URL.createObjectURL(fixedBlob);
...
function download() {
    dialog.showSaveDialog({
        ...
        if (!file.canceled) {
            ...
            blobToDataConverter(fixedBlob, file);
        }
    });
}
